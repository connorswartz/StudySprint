  ...
  const handleTimerComplete = async () => {
      try {
          ...
          if (response.status === 201) {
              ...
              await axios.patch(`http://localhost:8000/api/tasks/${se                    completed_sessions: selectedTask.completed_sessions + 1,
              });
          }
      } catch (error) {
      ...
      <CardBody className="items-center">
        {showModal ? (
          ...
          <>
            ...
            {selectedTask && (
              <p className="text-center text-white mb-4">{selectedTask}</p>
            )}
