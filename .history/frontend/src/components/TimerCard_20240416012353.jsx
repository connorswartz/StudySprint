...
const TimerCard = ({ selectedTask, onTaskUpdate }) => {
  ...
  const [prevRemainingTime, setPrevRemainingTime] = useState(duration);
  ...
  const handleOpenModal = (remainingTime) => {
    setIsPlaying(false);
      ...
      <CardBody className="items-center">
        {showModal ? (
          ...
          <>
            ...
            >
              {({ remainingTime }) => (
                <div onClick={() => handleOpenModal(remainingTime)}>{renderTime({ remainingTime })}</div>
              )}
            </CountdownCircleTimer>
            <div className="flex justify-center gap-4 mt-4">
              ...
              <Button onClick={() => handleOpenModal(duration)} color="secondary">
                Settings
              </Button>
            </div>
          </>
