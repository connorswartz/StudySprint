  ...
  const handleTimerComplete = useCallback(async () => {
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/sessions/', {
        ...
        starttime: 0, // Send the start time as 0 seconds
        endtime: duration, // Send the end time as the duration in seconds
  ...
  onClick={async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/sessions/', {
        ...
        task_id: selectedTask.task_id,
        starttime: 0, // Send the start time as 0 seconds
        endtime: duration - remainingTime, // Send the end time as the elapsed time in seconds
        date: new Date().toISOString().slice(0, 10),
      });
                    if (response.status === 201) {
                      setStartTime(null);
