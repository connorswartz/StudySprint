...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Generate a random task_id that doesn't exist in the api_task table
        while True:
            task_id = random.randint(1, 1000000)
            if not Task.objects.filter(task_id=task_id).exists():
                break

        task = Task.objects.create(task_id=task_id, task_name=task_name, report_id_id=report_id, user_id_id=user_id)
