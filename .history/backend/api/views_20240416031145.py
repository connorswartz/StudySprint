...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        # Get the maximum session_id from the existing sessions
        max_session_id = Session.objects.aggregate(Max('session_id'))['session_id__max']
        new_session_id = (max_session_id + 1) if max_session_id else 1

        # Keep incrementing the session_id until a unique value is found
        while True:
            try:
                serializer.save(session_id=new_session_id)
                break
